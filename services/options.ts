...
// ─── KQ150 위클리: API 코드 기반 주차 키 생성 (이번달+다음달) ─
...
// ─── KP200 위클리: 유효 탭 목록 + 첫 번째 board 데이터 함께 반환 ──
// 검증용 t2301 호출 결과를 재사용 → 첫 로드 시 중복 API 호출 없음
export async function fetchKP200ValidWeeklyKeys(token: string): Promise<{
  ...
  for (let monthOffset = 0; monthOffset <= 1; monthOffset++) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (day === 1) candidates.push({ key: `${year}${String(month + 1).padStart(2, '0')}_W${monCount}MON`, yyyymm: `W${monCount}MON`, label: `${monCount}주 월요일`, actualDate: date });
      if (day === 4) candidates.push({ key: `${year}${String(month + 1).padStart(2, '0')}_W${thuCount}THU`, yyyymm: `W${thuCount}THU`, label: `${thuCount}주 목요일`, actualDate: date });
    }
  }
  ...
  try {
    ...
    const board = Object.keys(putMap)
      .map(key => {
        ...
        return { actprice: Number(key), callCode: call?.shcode ?? '', callPrice: callP.price, callChange: callP.change, putCode: put.shcode, putPrice: putP.price, putChange: putP.change };
      })
      .filter(item => item.actprice > 0)
      .sort((a, b) => b.actprice - a.actprice);
    let futurePrice = 0;
...
export async function fetchKP200MonthlyBoard(token: string, yyyymm: string): Promise<OptionBoardResult> {
  try {
    ...
    const board: OptionBoardItem[] = Object.keys(putMap).map(actprice => {
      ...
      return { actprice: Number(actprice), callCode: call?.optcode ?? '', callPrice: Number(call?.price ?? 0), callChange: (callSign === '5' || callSign === '6') ? -callAbsChange : callAbsChange, putCode: put.optcode, putPrice: Number(put.price ?? 0), putChange: (putSign === '5' || putSign === '6') ? -putAbsChange : putAbsChange };
  ...
  } catch (e) { console.log('KP200 월물 전광판 조회 에러:', e); return { board: [], futurePrice: 0, jandatecnt: 0 }; }
}
...
export async function fetchKQ150MonthlyBoard(token: string, yyyymm: string): Promise<OptionBoardResult> {
  try {
    ...
    return { board, futurePrice: Number(summary.gmprice ?? 0), jandatecnt: Number(summary.jandatecnt ?? 0) };
  } catch (e) { console.log('KQ150 월물 전광판 조회 에러:', e); return { board: [], futurePrice: 0, jandatecnt: 0 }; }
}
...
export async function fetchKP200FutureList(token: string): Promise<{ shcode: string; hname: string }[]> {
  try {
    ...
    return (data?.t8467OutBlock ?? []).filter((item: any) => item.shcode?.startsWith('A')).map((item: any) => ({ shcode: item.shcode, hname: item.hname }));
  } catch { return []; }
}
...
export async function fetchKQ150FutureList(token: string): Promise<{ shcode: string; hname: string }[]> {
  try {
    ...
    return (data?.t8435OutBlock ?? []).filter((item: any) => item.shcode?.startsWith('A')).map((item: any) => ({ shcode: item.shcode, hname: item.hname }));
  } catch { return []; }
}